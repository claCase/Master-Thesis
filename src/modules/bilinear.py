...
class idx_to_Features(keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        for i in range(self.n_relations):
            self.layers.append(
                keras.layers.Dense(units=self.n_output, activation="relu")
            )
    ...
    def call(self, inputs, **kwargs):
        ...
        return tf.transpose(
            output, perm=[1, 0, 2, 3]
        )  # batches x relations x nodes x features
...
if __name__ == "__main__":
    ...
    relations = 5
    features = 4
    nodes_idx_to_onehot = np.zeros(
        shape=(batches, nodes, nodes)
    )  # batches x nodes x features
    idxs = np.arange(nodes)
    nodes_idx_to_onehot[:, idxs, idxs] = 1.0

    np_input = np.ones(
        shape=(batches, relations, nodes, features)
    )  # (batch, relations, nodes, features)
    adj = np.random.choice(
        [0, 1], batches * nodes * nodes * relations, p=[0.5, 0.5]
    ).reshape(batches, relations, nodes, nodes)
